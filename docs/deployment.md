...
Three things it cannot do for you, because they are secrets or decisions:
...
2. **Fill in the five `sync: false` variables** on the web service. The worker
   ...
   Three more are worth knowing about but are not asked for: `S3_ENDPOINT`
   (only for R2, B2 or another non-AWS store), `STRIPE_SECRET_KEY` and
   `EMAIL_PROVIDER` — add any of them in the dashboard when you want the
   feature.
